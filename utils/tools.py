...
def nms(bboxes, score_threshold, iou_threshold, sigma=0.3, method='nms'):
    """
:param bboxes:
     Assuming that there are N bboxes whose score is greater than score_threshold, then the shape of bboxes is (N, 6), and the storage format is (xmin, ymin, xmax, ymax, score, class) 
     Among them (xmin, ymin, xmax, ymax) are relative to the original input image, score = conf * prob, class is the index number of the category to which bbox belongs
     :return: best_bboxes
     Assuming there are N bboxes left after NMS, the shape of best_bboxes is (N, 6), and the storage format is (xmin, ymin, xmax, ymax, score, class)
     Among them (xmin, ymin, xmax, ymax) are relative to the original input image, score = conf * prob, class is the index number of the category to which bbox belongs
    """
...
def plot_box(bboxes, img, cfg, id = None, color=None, line_thickness=None):
    """
    显示图片img和其所有的bboxes
